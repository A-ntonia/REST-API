...
function buildNav(members) {
    let globalNav;
    let nav = '<h2><a href="index.html">Language courses API</a></h2>';
    const seen = {};
    const seenTutorials = {};
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    if (members.globals.length) { generate('API', [{kind: 'globalobj'}], globalUrl); }

    // index page displays information from package.json and lists files
    files = find({kind: 'file'});
    packages = find({kind: 'package'});

    generate('Language courses API',
        packages.concat(
            [{
                kind: 'mainpage',
                readme: opts.readme,
